...
import com.app.Repo.SessioneRepo;
...
public class StudentiController {
    ...
    @Autowired(required=true)
  	private SessioneRepo sessionerepo;
	...
	public String LoginStudente(@ModelAttribute("studente") Studente studente,HttpServletRequest request,Model model) 
	{
		...
		if(data.isPresent()==true) {
		 if(studente.getPassword().equals(data.get().getPassword())) {
			...
			session.setAttribute("username",studente.getEmail());
			String sessionId = session.getId();
			
			System.out.println(sessionId);
			sessione=new Sessione(sessionId,studente);
			sessionerepo.save(sessione);
			//model.addAttribute("sessione",sessionId);
	...
	@PostMapping("/Registrastudente")
      public String RegistraStudente(@ModelAttribute("studente") Studente studente,Model model)
		...
		public String Logout(HttpServletRequest request) {
			...
			sessionerepo.deleteById(session.getId());
			if (session!= null) {
				session.invalidate();
				}
			return "redirect:/";
